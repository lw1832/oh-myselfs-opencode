...
export async function fetchAvailableModels(
	...
	const skipClientFetch =
		options?.skipClientFetch === true || process.env[OFFLINE_MODELS_ENV] === "1"
	...
	if (skipClientFetch) {
		...
		if (!connectedProvidersUnknown) {
			log("[fetchAvailableModels] skipClientFetch: using connected providers from cache only")
		}
	} else if (connectedProvidersUnknown && client) {
	...
	if (connectedProvidersUnknown) {
		if (!skipClientFetch && client?.model?.list) {
	...
	if (!existsSync(cacheFile)) {
		...
		if (skipClientFetch) {
			log("[fetchAvailableModels] skipClientFetch: returning empty set (no cache)")
			return modelSet
		}
	} else {
		try {
