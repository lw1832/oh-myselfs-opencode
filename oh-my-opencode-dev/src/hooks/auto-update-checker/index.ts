...
export function createAutoUpdateCheckerHook(ctx: PluginInput, options: AutoUpdateCheckerOptions = {}) {
  ...
  const skipModelListFetch = disableModelListFetch || process.env[OFFLINE_MODELS_ENV] === "1"
...
async function updateAndShowConnectedProvidersCacheStatus(ctx: PluginInput, skipFetch = false): Promise<void> {
  if (skipFetch) {
    return
  }
