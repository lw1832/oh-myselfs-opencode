...
| `src/tools/grep/downloader.ts` | `https://github.com/BurntSushi/ripgrep/releases/download/${RG_VERSION}/${filename}`（RG_VERSION=14.1.1） | 本地未安装 ripgrep 且工具需要时自动下载 |
...
| 文件路径 | 服务 URL | 用途 |
|----------|----------|------|
| `src/mcp/websearch.ts` | `https://mcp.exa.ai/mcp?tools=web_search_exa`（可选 `EXA_API_KEY`）或 `https://mcp.tavily.com/mcp/`（需 `TAVILY_API_KEY`） | Exa / Tavily 网络搜索 |
...
| 文件路径 | 目标 | 触发条件 |
|----------|------|----------|
| `src/shared/tmux/tmux-utils.ts` | `fetch(healthUrl)`，其中 `healthUrl = new URL("/health", serverUrl)`；`serverUrl` 默认 `http://localhost:${defaultPort}`（见 `src/features/tmux-subagent/manager.ts`） | 启用 Tmux 集成且检查本地 OpenCode/子智能体服务是否存活时 |
...
| `src/shared/connected-providers-cache.ts` | `updateConnectedProvidersCache(client)` 调用 `client.provider.list()` 与 `client.model.list()` 并写缓存 | `session.created` 时由 auto-update-checker 等触发 |
...
- `src/agents/librarian.ts` 中描述的 webfetch 行为（如 sitemap、doc page）由 **Librarian agent 在运行时** 根据用户配置的文档 URL 发起，非插件启动时自动请求。
...
| `.github/workflows/publish.yml` / `publish-platform.yml` | CI 中检查版本、发布 npm 包 |
| `.github/workflows/lint-workflows.yml` | 下载 actionlint 等 CI 工具 |
...
- **callback-server.ts** 中的 `fetch(request)`：为本地 OAuth 回调服务器处理请求，**不主动访问外网**。
...
| 版本/更新 | registry.npmjs.org | 自动更新检查、安装向导解析版本、getPluginNameWithVersion |
| 内置 MCP | mcp.exa.ai, mcp.tavily.com, mcp.context7.com, mcp.grep.app | 启用并调用 websearch/context7/grep_app 时 |
| MCP OAuth | 用户配置的 MCP server 域名（HTTPS） | `auth login` 及 OAuth 发现/DCR/换 token |
| 本地健康检查 | localhost（默认） | Tmux 集成检查本地服务 `/health` |
| 发布/CI | registry.npmjs.org, raw.githubusercontent.com 等 | 仅维护者构建/发布与 GitHub Actions |
...
2. **禁用 models.dev 间接触发**：在 `oh-my-opencode.json` 中设置 `experimental.disable_model_list_fetch: true`，插件将不再调用 `client.provider.list()` 与 `client.model.list()`，仅使用本地缓存（`~/.cache/oh-my-opencode/`、`~/.cache/opencode/models.json`）。也可设置环境变量 `OH_MY_OPENCODE_OFFLINE_MODELS=1` 达到同样效果（无需改配置）。离线使用前需在有网环境至少运行一次以生成缓存，或从它机拷贝。
3. **避免触发二进制下载**：提前在能联网的环境安装好 ripgrep、ast-grep，或安装 comment-checker 所需二进制并放入缓存目录（如 `~/.cache/oh-my-opencode/bin` 或 Windows `%LOCALAPPDATA%\oh-my-opencode\bin`），避免在离线环境首次调用时下载。
...
6. **安装**：在离线环境安装时，若安装向导会调用 `fetchLatestVersion` / `fetchNpmDistTags`，可能失败；可手动编辑 `opencode.json` 与 `oh-my-opencode.json` 完成安装，或使用内网安装包与安装程序（若有配套文档与脚本）。

---

*报告基于对当前仓库的静态代码与配置遍历生成，未包含 OpenCode 宿主或系统其它进程的联网行为。*
